...
| `ir_im_isq` | lexicographically minimize `(IdleRange(P*), maximum participant idle, squared idle sum)` (MaxSAT only) |
...
Participant-specific idle caps remain available as an optional sensitivity
study, but they are disabled in the baseline and final-tier comparisons. To
enable them explicitly, add
`--participant-idle-cap-rule interpolated_bounds` and an exact rational
`--participant-idle-cap-alpha`. For each
participant, the implementation computes a shortest matching-feasible unary
interval `LB_p`, the widest unary-domain span `UB_p`, and

```text
b_p = floor(LB_p + alpha * (UB_p - LB_p)).
```

It then adds the hard clause `I_p <= b_p`. Bounds use only session, fixed, and
forbidden-slot domains, so the legacy Full model and Reduced compact model use
the same cap. Every detailed row records all lower bounds, upper bounds, caps,
alpha, and the number of cap clauses.
...
- `src/ORG_new.py`: the strongest legacy paper-style MaxSAT baseline. Its
  original IR cell is preserved, and its per-slot/cardinality encoding also
  supports IR-IM-IS and IR-IM-ISQ, with optional participant idle caps.
...
## Balanced-scheduling journal campaigns

The experiment suite first compares the models on their native feasible set.
The optional cap sensitivity study remains separate and is deferred until the
uncapped baseline has been evaluated.

| Config | Purpose | Jobs |
|---|---:|---:|
| `compact_objectives.json` | reference/A/C/D/F compact ablation for BG-d2, IR, and uncapped IR-IM-IS | 1,890 |
| `idle_cap_sensitivity.json` | deferred optional cap study at alpha 1/2, 3/4, and 1 | 378 |
| `main_objectives.json` | 2022 vs compact, three primary objectives, three repetitions | 2,268 rows; 378 reused, 1,890 new runs |
| generated by `Select_Final_Tier_Experiment.py` | IR-IM-ISQ on the selected model; reuse main IR-IM-IS results | 378 |

Run the small gates first:

```bash
bash scripts/run_journal_gcp.sh compact-objectives-smoke
bash scripts/run_journal_gcp.sh main-objectives-smoke
```

Then run the frozen campaigns in the order below. Each command remains
append-only and resumable, and accepts the existing
`SHARD_COUNT`/`SHARD_INDICES` environment variables. Sharded outputs are
written below a `shard-*-of-*` subdirectory. `main-objectives` imports the
matching BG-d2 A+C+F, IR C+D+F, and IR-IM-IS C+D+F repetition-1 rows from
`compact-objectives`; it executes only the remaining 1,890 jobs. With
sharding, each VM reuses its matching local compact shard by default. Set
`COMPACT_OBJECTIVES_OUTPUT` when using a merged source or a non-default path.
Merge every main-objective shard before running `final-tier`, and point
`MAIN_OBJECTIVES_OUTPUT` to that merged directory. The final-tier campaign can
then be sharded independently with the same variables.
...
are copied into the main archive with `result_origin=reused` and source
campaign, plan, run-key, configuration, repetition, and record-hash fields.
The runner rejects a missing, incomplete, configuration-mismatched, or
environment-incompatible compact source instead of silently rerunning those
cells.

`final-tier` first validates the complete main campaign. It ranks the two
IR-IM-IS configurations by exact solved coverage, PAR-2, median peak RSS, and
configuration ID as a deterministic tie-break. It writes the selected config
and a selection report below `outputs/journal/`, then runs only IR-IM-ISQ with
the selected model and uncapped feasible set. IR-IM-IS results are read from
the completed main campaign and are not run again.

The deferred cap study can later be run independently with
`cap-sensitivity-smoke` followed by `cap-sensitivity`; its rows are not part of
the baseline comparison or model selection.

For a read-only plan check without the wrapper:

```bash
python3 src/Journal_Experiment.py \
  --config journal_configs/main_objectives.json \
  --output-dir /tmp/b2b-main-plan \
  --plan-only
```
