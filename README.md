...
| `main_objectives.json` | 2022 vs compact, three primary objectives, three repetitions | 2,268 |
...
Then run the frozen campaigns. Each command remains append-only and resumable,
and accepts the existing `SHARD_COUNT`/`SHARD_INDICES` environment variables.
Sharded outputs are written below a `shard-*-of-*` subdirectory. Merge every
main-objective shard before running `final-tier`, and point
